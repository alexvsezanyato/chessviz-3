...
void Moves::ParseRegex(std::smatch* parts, Move* move1, Move* move2)
{
	...
	if (parts->str(9).size()) move1->postfix = parts->str(9).at(0);

	if (!parts->str(10).size()) return;
	// ..
    ...
    if (parts->str(17).size()) move2->postfix = parts->str(17).at(0);
}
...
bool Moves::Read()
{
	...
	{
		std::string regmv = "([K, Q, R, B, N])?([a-h])([1-8])([-,x])([a-h])([1-8])([K, Q, R, B, N, #, +]?)";
		...
		if (std::regex_match(buffer, parts, regex))
		{
			...
			if (move2.alias != "null") moves.push_back(move2), count++;
		}
		else return false;
	}
	return true;
...
Moves::Moves(std::string path)
{
	...
	if (stream)
	{
		if (!Read()) std::cout << "Invalid input data \n";
		stream.close();
	}
	else std::cout << "Can't open file \"" << path << "\"\n";
}